...
private extension InboxNotesStore {
    ...
    ///
    func upsertStoredInboxNotesInBackground(readOnlyInboxNotes: [MateNetworking.InboxNote],
                                            siteID: Int64,
    ...
    ///
    func upsertStoredInboxNotes(readOnlyInboxNotes: [MateNetworking.InboxNote],
                                in storage: StorageType,
                                siteID: Int64) {
        for inboxNote in readOnlyInboxNotes {
            let storageInboxNote = storage.loadInboxNote(siteID: siteID, id: inboxNote.id) ?? storage.insertNewObject(ofType: MateStorage.InboxNote.self)
    ...
    ///
    func handleInboxActions(_ readOnlyInboxNote: MateNetworking.InboxNote, _ storageInboxNote: MateStorage.InboxNote, _ storage: StorageType) {
        ...
        for readOnlyInboxAction in readOnlyInboxNote.actions {
            let newStorageInboxAction = storage.insertNewObject(ofType: MateStorage.InboxAction.self)
