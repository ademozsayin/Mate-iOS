...
public class OnsaAnalytics: Analytics {
    ...
    ///
    init(analyticsProvider: AnalyticsProvider & FiableAnalyticsTracker) {
        self.analyticsProvider = analyticsProvider
        FiableAnalytics.registerTracker(analyticsProvider)
    }
}
...
//
public extension OnsaAnalytics {
    ...
    ///   - error: the error to track
    ///
    func track(_ stat: OnsaAnalyticsStat, properties passedProperties: [AnyHashable: Any]?, error: Error?) {
        guard userHasOptedIn == true else {
            return
        }
...
//
private extension OnsaAnalytics {
    ...
    ///
    func updatePropertiesIfNeeded(for stat: OnsaAnalyticsStat, properties: [AnyHashable: Any]?) -> [AnyHashable: Any]? {
        guard stat.shouldSendSiteProperties, ServiceLocator.stores.isAuthenticated else {
            return properties
        }

        var updatedProperties = properties ?? [:]
        let site = 1//ServiceLocator.stores.sessionManager.defaultSite
...
//                return "\(FiableAnalyticsEvent.Widgets.Name.todayStats.rawValue)-\(widgetInfo.family)"
//            case FiableConstants.appLinkWidgetKind:
//                return FiableAnalyticsEvent.Widgets.Name.appLink.rawValue
            default:
