...
extension MateAnalyticsStat {
    enum LoginOnboarding {
        ...
        static func loginOnboardingShown() -> MateAnalyticsEvent {
            MateAnalyticsEvent(name: .loginOnboardingShown, properties: [:])
        }
        ...
        static func loginOnboardingNextButtonTapped(isFinalPage: Bool) -> MateAnalyticsEvent {
            MateAnalyticsEvent(name: .loginOnboardingNextButtonTapped, properties: [Keys.isFinalFeaturePage: isFinalPage])
        }

        /// Tracked when the user taps on the “Skip” button on the login onboarding screen to enter the prologue screen.
        static func loginOnboardingSkipButtonTapped() -> MateAnalyticsEvent {
            MateAnalyticsEvent(name: .loginOnboardingSkipButtonTapped, properties: [:])
        }
    }
}
