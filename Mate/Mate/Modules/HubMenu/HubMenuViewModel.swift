...
final class HubMenuViewModel: ObservableObject {
    ...
    var email: String {
        guard let useremail = stores.sessionManager.defaultAccount?.name else {
//            return nil
            return stores.sessionManager.defaultAccount?.email ?? "Mate User"
        }
        return useremail
    }
    ...
    @Published private(set) var storeURL = FiableConstants.URLs.blog.asURL()

    @Published private(set) var woocommerceAdminURL = FiableConstants.URLs.blog.asURL()
