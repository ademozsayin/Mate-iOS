...
final class ReviewsViewController: UIViewController, GhostableViewController {
    ...
    lazy var ghostTableViewController = GhostTableViewController(options: GhostTableViewOptions(
        sectionHeaderVerticalSpace: .large,
    ...
    //
    convenience init() {
        self.init(viewModel: ReviewsViewModel(
            data: ReviewsDataSource(
...
private extension ReviewsViewController {
    ...
    func displayEmptyViewController() {
        ...
        let emptyStateConfig = EmptyStateViewController.Config.withLink(
            message: NSAttributedString(string: Localization.EmptyState.message),
            image: .emptyReviewsImage,
            details: Localization.EmptyState.detail,
            linkTitle: Localization.EmptyState.action,
            linkURL: FiableConstants.URLs.productReviewInfo.asURL())
