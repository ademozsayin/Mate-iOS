...
final class ProductCategoryListViewModel {
    ...
    ///
    @Published private(set) var selectedCategories: [MateCategory]
...
//        categoryViewModels = enrichingDataSource?.enrichCategoryViewModels(baseViewModels) ?? baseViewModels
    ...
    func deleteCategory(id: Int64) async {
        deletionFailure = nil
        let selectedItem = selectedCategories.first(where: { $0.id != id })
        do {
            // optimistic deletion
            categoryViewModels.removeAll(where: { $0.categoryID == id })
            // removes the category from the selected list if exists
            selectedCategories = selectedCategories.filter { $0.id != id }
...
//            synchronizeAllCategories()
            ...
            if let selectedItem {
                ...
                initiallySelectedIDs.append(Int64(selectedItem.id))
            }
