...
final class UpgradesViewModel: ObservableObject {
    ...
    func prepareViewModel() async {
        do {
            guard let site = stores.sessionManager.defaultAccount else {
                throw PrePurchaseError.fetchError
            }

            guard (site.userID != 0) else {
            ...
            plans = try await retrievePlanDetailsIfAvailable(
                [.essentialMonthly,
                .essentialYearly,
                .performanceMonthly,
                .performanceYearly,
                .debugPerformanceMonthly
                ],
                from: wpcomPlans,
    ...
    private func observeViewStateAndTrackAnalytics() {
        $upgradeViewState.sink { [weak self] state in
            ...
            case .prePurchaseError(let error):
                print("self?.analytics.track(event: .InAppPurchases.planUpgradePrePurchaseFailed(error: error))")
            case .purchaseUpgradeError(let error):
                print("self?.analytics.track(event: .InAppPurchases.purchaseUpgradeError(error: error))")
