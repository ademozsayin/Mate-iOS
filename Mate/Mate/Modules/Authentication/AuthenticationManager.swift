...
extension AuthenticationManager: FiableAuthenticatorDelegate {
    @MainActor
    func loginWith(email: String, password: String, onCompletion: @escaping (Result<MateNetworking.OnsaTokenData, any Error>) -> Void) async {
        
        do {
            let data = try await withCheckedThrowingContinuation { continuation in
                let action = GetStartedAction.getToken(email: email, password: password) { result in
...
//        coordinator.start()
    ...
    func presentAppEmailExistError(error:String, in viewController: UIViewController) {
