...
private extension SiteAddressViewController {
    ...
    func loadRows() {
        ...
        if FiableAuthenticator.shared.configuration.displayHintButtons {
    ...
    func configureInstructionLabel(_ cell: TextLabelTableViewCell) {
        cell.configureLabel(text: FiableAuthenticator.shared.displayStrings.siteLoginInstructions, style: .body)
    }
    ...
    func configureTextField(_ cell: TextFieldTableViewCell) {
        cell.configure(withStyle: .url,
                       placeholder: FiableAuthenticator.shared.displayStrings.siteAddressPlaceholder)
    ...
    func configureTextLinkButton(_ cell: TextLinkButtonTableViewCell) {
        cell.configureButton(text: FiableAuthenticator.shared.displayStrings.findSiteButtonTitle)
    ...
    func pushCustomUI(_ customUI: UIViewController) {
        ...
        if FiableAuthenticator.shared.delegate?.supportActionEnabled == true {
...
private extension SiteAddressViewController {
    ...
    func validateForm() {
        ...
        loginFields.siteAddress = FiableAuthenticator.baseSiteURL(string: loginFields.siteAddress)
...
//                    FiableAuthenticator.shared.delegate?.troubleshootSite(nil, in: self.navigationController)
...
//            })
    ...
    func fetchSiteInfo() {
        let baseSiteUrl = FiableAuthenticator.baseSiteURL(string: loginFields.siteAddress)
        ...
        let successBlock: (WordPressComSiteInfo) -> Void = { [weak self] siteInfo in
            ...
            self.configureViewLoading(false)
            if siteInfo.isWPCom && FiableAuthenticator.shared.delegate?.allowWPComLogin == false {
...
//        })
    ...
    func presentNextControllerIfPossible(siteInfo: WordPressComSiteInfo?) {
        ...
        guard isSiteDiscovery == false else {
            FiableAuthenticator.shared.delegate?.troubleshootSite(siteInfo, in: navigationController)
            return
        }
        ...
        FiableAuthenticator.shared.delegate?.shouldPresentUsernamePasswordController(for: siteInfo, onCompletion: { (result) in
            switch result {
...
//    }
    ...
    func showSelfHostedUsernamePassword() {
        ...
        guard let vc = SiteCredentialsViewController.instantiate(from: .siteAddress) else {
            print("Failed to navigate from SiteAddressViewController to SiteCredentialsViewController")
            return
        }
    ...
    func showWPUsernamePassword() {
        ...
        guard let vc = LoginUsernamePasswordViewController.instantiate(from: .login) else {
            print("Failed to navigate from SiteAddressViewController to LoginUsernamePasswordViewController")
            return
        }
    ...
    func showGetStarted() {
        guard let vc = GetStartedViewController.instantiate(from: .getStarted) else {
            print("Failed to navigate from SiteAddressViewController to GetStartedViewController")
            return
        }
