...
//        loginFacade.loginToWordPressDotCom(withUser: loginFields.nonceUserID, authType: authType, twoStepCode: code, twoStepNonce: nonce)
    ...
    func finishedLogin(withNonceAuthToken authToken: String) {
        let wpcom = WordPressComCredentials(authToken: authToken, multifactor: true, siteURL: loginFields.siteAddress)
...
//                                                  userHandle: credential.userID)
    ...
    func authorizationController(controller: ASAuthorizationController, didCompleteWithError error: Error) {
        print("Error signing challenge: \(error.localizedDescription)")
...
//
//        if #available(iOS 16, *), FiableAuthenticator.shared.configuration.enablePasskeys, loginFields.nonceInfo?.nonceWebauthn.isEmpty == false {
//            rows.append(.spacer(4))
...
//        }
    ...
    func configureInstructionLabel(_ cell: TextLabelTableViewCell) {
        cell.configureLabel(text: FiableAuthenticator.shared.displayStrings.twoFactorInstructions)
    }

    /// Configure the alternate instruction cell.
    ///
    func configureAlternateInstructionLabel(_ cell: TextLabelTableViewCell) {
        cell.configureLabel(text: FiableAuthenticator.shared.displayStrings.twoFactorOtherFormsInstructions)
    }
    ...
    func configureTextField(_ cell: TextFieldTableViewCell) {
        cell.configure(withStyle: .numericCode,
                       placeholder: FiableAuthenticator.shared.displayStrings.twoFactorCodePlaceholder)
    ...
    func configureTextLinkButton(_ cell: TextLinkButtonTableViewCell) {
        cell.configureButton(text: FiableAuthenticator.shared.displayStrings.textCodeButtonTitle, icon: .phoneIcon)
    ...
    func configureEnterSecurityKeyLinkButton(_ cell: TextLinkButtonTableViewCell) {
        cell.configureButton(text: FiableAuthenticator.shared.displayStrings.securityKeyButtonTitle,
                             icon: .keyIcon,
