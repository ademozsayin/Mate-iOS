...
class GravatarEmailTableViewCell: UITableViewCell {
    ...
    public func configure(withEmail email: String?, andPlaceholder placeholderImage: UIImage? = nil, hasBorders: Bool = false) {
        gravatarImageView?.tintColor = FiableAuthenticator.shared.unifiedStyle?.borderColor ?? FiableAuthenticator.shared.style.primaryNormalBorderColor
        emailLabel?.textColor = FiableAuthenticator.shared.unifiedStyle?.gravatarEmailTextColor ?? FiableAuthenticator.shared.unifiedStyle?.textSubtleColor ?? FiableAuthenticator.shared.style.subheadlineColor
