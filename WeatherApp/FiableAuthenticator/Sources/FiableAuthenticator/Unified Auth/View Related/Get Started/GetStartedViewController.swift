...
class GetStartedViewController: LoginViewController, NUXKeyboardResponder {
    ...
    var source: SignInSource? {
        didSet {
            FiableAuthenticator.shared.signInSource = source
        }
    }
    ...
    private lazy var continueButton: NUXButton = {
        ...
        button.customizeFont(WPStyleGuide.fixedBoldNotoFontWithSize(15))
        return button
    }()
    ...
    private lazy var whatisWPCOMButton: UIButton = {
        ...
        button.titleLabel?.font = WPFontManager.notoRegular(ofSize: 15)
    ...
    override var preferredStatusBarStyle: UIStatusBarStyle {
        return FiableAuthenticator.shared.unifiedStyle?.statusBarStyle ??
            FiableAuthenticator.shared.style.statusBarStyle
    }
...
private extension GetStartedViewController {
    ...
    func configureNavBar() {
        navigationItem.title = FiableAuthenticator.shared.displayStrings.getStartedTitle
    ...
    func configureDivider() {
        ...
        let color = FiableAuthenticator.shared.unifiedStyle?.borderColor ?? FiableAuthenticator.shared.style.primaryNormalBorderColor
    ...
    func loadRows() {
        ...
        if let authenticationDelegate = FiableAuthenticator.shared.delegate, authenticationDelegate.wpcomTermsOfServiceEnabled {
            rows.append(.tos)
        }
    ...
    func configure(_ cell: UITableViewCell, for row: Row, at indexPath: IndexPath) {
        ...
        default:
            print("Error: Unidentified tableViewCell type found.")
        }
    }
    ...
    func configureInstructionLabel(_ cell: TextLabelTableViewCell) {
        cell.configureLabel(text: FiableAuthenticator.shared.displayStrings.getStartedInstructions)
    }
    ...
    func configureEmailField(_ cell: TextFieldTableViewCell) {
        cell.configure(withStyle: .email,
                       placeholder: FiableAuthenticator.shared.displayStrings.emailAddressPlaceholder,
    ...
    func configureTextWithLink(_ cell: TextWithLinkTableViewCell) {
        cell.configureButton(markedText: FiableAuthenticator.shared.displayStrings.loginTermsOfService)
...
private extension GetStartedViewController {
    ...
    func validateForm() {
        ...
        authenticationDelegate.checkIfEmailExist(email: loginFields.username, onCompletion: { result in
            print(" authenticationDelegate.checkIfEmailExist callback")
...
//        })
    ...
    func showPasswordView() {
        guard let vc = PasswordViewController.instantiate(from: .password) else {
            print("Failed to navigate to PasswordViewController from GetStartedViewController")
            return
        }
    ...
    func handleLoginError(_ error: Error) {
        ...
        let errorCode = "sss"//userInfo[WordPressComRestApi.ErrorKeyErrorCode] as? String
        ...
        if configuration.enableSignUp, errorCode == "unknown_user" {
            ...
            guard let authenticationDelegate = FiableAuthenticator.shared.delegate,
...
//        })
    ...
    private func didRequestSignupLink() {
        guard let vc = SignupMagicLinkViewController.instantiate(from: .unifiedSignup) else {
            print("Failed to navigate from UnifiedSignupViewController to SignupMagicLinkViewController")
            return
        }
...
//        })
    ...
    private func buildInvalidEmailAlert(title: String, message: String) -> UIAlertController {
        ...
        alert.addActionWithTitle(helpActionTitle,
                                 style: .cancel,
                                 handler: { _ in
                                    FiableAuthenticator.shared.delegate?.presentSupportRequest(from: self, sourceTag: .loginEmail)
        })
    ...
    func showSelfHostedWithError(_ error: Error) {
        ...
        guard let vc = SiteCredentialsViewController.instantiate(from: .siteAddress) else {
            print("Failed to navigate to SiteCredentialsViewController from GetStartedViewController")
            return
        }
...
private extension GetStartedViewController {
    ...
    @objc func googleTapped() {
        ...
        guard let toVC = GoogleAuthViewController.instantiate(from: .googleAuth) else {
            print("Failed to navigate to GoogleAuthViewController from GetStartedViewController")
            return
        }
...
private extension GetStartedViewController {
    enum ButtonConfiguration {
        enum Continue {
            static let title = FiableAuthenticator.shared.displayStrings.continueButtonTitle
