...
//        FiableAuthenticator.track(.loginURLFormViewed)
    }
    ...
    @objc func localizeControls() {
        instructionLabel?.text = FiableAuthenticator.shared.displayStrings.siteLoginInstructions
...
//            FiableAuthenticator.track(.loginFailedToGuessXMLRPC, error: error)
...
//        return err
    ...
    @objc func showWPUsernamePassword() {
        ...
        guard let vc = LoginUsernamePasswordViewController.instantiate(from: .login) else {
            print("Failed to navigate from LoginSiteAddressViewController to LoginUsernamePasswordViewController")
                return
        }
...
//        FiableAuthenticator.track(.loginURLHelpScreenViewed)
    }
