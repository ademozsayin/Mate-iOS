...
class NUXLinkAuthViewController: LoginViewController {
    ...
    func syncAndContinue(authToken: String, flow: Flow, isJetpackConnect: Bool) {
        let wpcom = WordPressComCredentials(authToken: authToken, multifactor: false, siteURL: "https://wordpress.com")
...
//                FiableAuthenticator.track(.createdAccount, properties: ["source": "email"])
//                FiableAuthenticator.track(.signupMagicLinkSucceeded)
//            case .login:
//                FiableAuthenticator.track(.loginMagicLinkSucceeded)
//            }
        }
    }
}
