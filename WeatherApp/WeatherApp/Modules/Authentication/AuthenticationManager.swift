...
//    private let switchStoreUseCase: SwitchStoreUseCaseProtocol?
    ...
    ///
    func initialize() {
        FiableAuthenticator.initializeWithCustomConfigs()
        FiableAuthenticator.shared.delegate = self
    }
    ...
    func authenticationUI() -> UIViewController {
        let loginViewController: UIViewController = {
            let loginUI = FiableAuthenticator.loginUI(onLoginButtonTapped: { [weak self] in
...
// MARK: - FiableAuthenticator Delegate
//
extension AuthenticationManager: FiableAuthenticatorDelegate {
    func checkIfEmailExist(email: String, onCompletion: @escaping (Result<Networking.EmailCheckData, any Error>) -> Void) {
        stores.dispatch(AccountAction.checkEmail(email: email, onCompletion: onCompletion))
    }
    
//    func checkIfEmailExist(email: String, onCompletion: @escaping (Networking.EmailCheckData) -> Void) {
//        
//        
//    }
//    
//    
//    
//    func checkIfEmailExist(email: String, onCompletion: @escaping () -> Void) {
//        
//        stores.dispatch(AccountAction.checkEmail(email: email, onCompletion: { result in
//            switch result {
//            case .success(let res):
//                if res.email == email {
//                    print("navigate to password coordinator")
//                } else {
//                    print("given email is not equal server email")
//                }
//            case .failure(let error):
//               print(error)
//                DDLogError("⛔️ Failed : \(error.localizedDescription)")
//            }
//        }))
//
//    }
    ...
    var supportEnabled: Bool {
       return false // return ZendeskProvider.shared.zendeskEnabled
    }
    ...
    ///
    func shouldPresentUsernamePasswordController(for siteInfo: WordPressComSiteInfo?, onCompletion: @escaping (FiableAuthenticatorResult) -> Void) {
...
//            ))
        ...
        guard let site = siteInfo, site.isWP else {
            let authenticationResult: FiableAuthenticatorResult = .injectViewController(value: noWPUI)
        ...
        if site.isWPCom || site.isJetpackConnected {
            let authenticationResult: FiableAuthenticatorResult = .presentEmailController
            onCompletion(authenticationResult)
        } else {
            let authenticationResult: FiableAuthenticatorResult = .presentPasswordController(value: true)
            onCompletion(authenticationResult)
        }
    }
...
//                return FiableAuthenticator.showLoginForJustWPCom(
...
//            FiableAuthenticator.showVerifyEmailForWPCom(
//                from: navigationController,
//                xmlrpc: xmlrpc,
...
//            FiableAuthenticator.track(.signedIn)
//
//            // navigates to home screen immediately with a placeholder store ID
//            self.startStorePicker(with: FiableConstants.placeholderStoreID, in: navigationController)
//        }
