    ...
    static func siteAddressHelpController(
        ...
        let moreHelpButton = ButtonConfig(Strings.moreHelp) { controller, _ in
            controller.dismiss(animated: true) {
                ...
                guard FiableAuthenticator.shared.delegate?.supportEnabled == true,
                ...
                moreHelpTapped?()
                FiableAuthenticator.shared.delegate?.presentSupportRequest(from: viewController, sourceTag: sourceTag)
            }
        }

        let image = FiableAuthenticator.shared.displayImages.siteAddressModalPlaceholder
...
//        print(message)
        ...
        if sourceTag == .jetpackLogin && error.domain == FiableAuthenticator.errorDomain && error.code == NSURLErrorBadURL {
            if FiableAuthenticator.shared.delegate?.supportEnabled == true {
...
//        }
        ...
        let moreHelpButton = ButtonConfig(Strings.moreHelp) { controller, _ in
            controller.dismiss(animated: true) {
                guard let sourceViewController = UIApplication.shared.delegate?.window??.topmostPresentedViewController,
                    let authDelegate = FiableAuthenticator.shared.delegate
                else {
                    return
                }
    ...
    static func alertForGenericErrorMessageWithHelpButton(_ message: String, loginFields: LoginFields, sourceTag: WordPressSupportSourceTag, onDismiss: (() -> ())? = nil) -> FancyAlertViewController {
        ...
        if FiableAuthenticator.shared.delegate?.supportEnabled == false {
            print("Error Alert: Support not enabled. Hiding Help button.")
        } else {
            moreHelpButton = ButtonConfig(Strings.moreHelp) { controller, _ in
                controller.dismiss(animated: true) {
                    ...
                    guard let appDelegate = UIApplication.shared.delegate,
                        ...
                        FiableAuthenticator.shared.delegate?.supportEnabled == true
                        else {
                            return
                    }
