...
final class SubheadlineButton: UIButton {
    override func traitCollectionDidChange(_ previousTraitCollection: UITraitCollection?) {
        ...
        if previousTraitCollection?.preferredContentSizeCategory != traitCollection.preferredContentSizeCategory {
            ...
            setTitleColor(FiableAuthenticator.shared.style.textButtonColor, for: .normal)
            setTitleColor(FiableAuthenticator.shared.style.textButtonHighlightColor, for: .highlighted)
        }
    }
}
...
extension WPStyleGuide {
    ...
    class func configureTextButton(_ button: UIButton) {
        button.setTitleColor(FiableAuthenticator.shared.style.textButtonColor, for: .normal)
        button.setTitleColor(FiableAuthenticator.shared.style.textButtonHighlightColor, for: .highlighted)
    }
    ...
    class func colorForErrorView(_ opaque: Bool) -> UIColor {
        ...
        return .red//UIColor(fromRGBAColorWithRed: 17.0, green: 17.0, blue: 17.0, alpha: alpha)
    }
    ...
    class func googleLoginButton() -> UIButton {
        ...
        let attrStrNormal = googleButtonString(baseString, linkColor: FiableAuthenticator.shared.style.textButtonColor)
        let attrStrHighlight = googleButtonString(baseString, linkColor: FiableAuthenticator.shared.style.textButtonHighlightColor)
    ...
    class func formattedSignInWithSiteCredentialsString() -> NSAttributedString {
        let title = FiableAuthenticator.shared.displayStrings.signInWithSiteCredentialsButtonTitle
        let globe = UIImage.gridicon(.globe)
        let image = globe.imageWithTintColor(FiableAuthenticator.shared.style.placeholderColor) ?? globe
    ...
    class func selfHostedLoginButton(alignment: UIControl.NaturalContentHorizontalAlignment = .leading) -> UIButton {
        ...
        let style = FiableAuthenticator.shared.style

        let button: UIButton

        if FiableAuthenticator.shared.configuration.showLoginOptions {
    ...
    class func signupTermsButton() -> UIButton {
        let unifiedStyle = FiableAuthenticator.shared.unifiedStyle
        let originalStyle = FiableAuthenticator.shared.style
        let baseString = FiableAuthenticator.shared.displayStrings.signupTermsOfService
...
//        button.titleLabel?.textAlignment = .center
    ...
    private class func textButton(normal normalString: NSAttributedString, highlighted highlightString: NSAttributedString, font: UIFont, alignment: UIControl.NaturalContentHorizontalAlignment = .leading, forUnified: Bool = false) -> UIButton {
        ...
        button.setTitleColor(FiableAuthenticator.shared.style.subheadlineColor, for: .normal)
    ...
    private class func selfHostedButtonString(_ buttonText: String, linkColor: UIColor) -> NSAttributedString {
        ...
        if let originalDomainsIcon = UIImage.gridicon(.domains).imageWithTintColor(FiableAuthenticator.shared.style.placeholderColor) {
