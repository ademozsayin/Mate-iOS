...
class GoogleAuthenticator: NSObject {
    ...
    private let authConfig = FiableAuthenticator.shared.configuration
...
//    }()
    ...
    private weak var authenticationDelegate: FiableAuthenticatorDelegate? = {
    ...
    func createGoogleAccount(loginFields: LoginFields) {
        ...
        guard let token = loginFields.meta.socialServiceIDToken else {
            print("GoogleAuthenticator - createGoogleAccount: Failed to get Google account information.")
            return
        }
