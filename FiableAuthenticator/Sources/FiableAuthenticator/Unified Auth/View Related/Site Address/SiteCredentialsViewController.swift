...
final class SiteCredentialsViewController: LoginViewController {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.title = FiableAuthenticator.shared.displayStrings.logInTitle
        if isDismissible {
    ...
    override var preferredStatusBarStyle: UIStatusBarStyle {
        return FiableAuthenticator.shared.unifiedStyle?.statusBarStyle ?? FiableAuthenticator.shared.style.statusBarStyle
    }
...
private extension SiteCredentialsViewController {
    ...
    func configureInstructionLabel(_ cell: TextLabelTableViewCell) {
        ...
        let text = String.localizedStringWithFormat(FiableAuthenticator.shared.displayStrings.siteCredentialInstructions, displayURL)
    ...
    func configureUsernameTextField(_ cell: TextFieldTableViewCell) {
        cell.configure(withStyle: .username,
                       placeholder: FiableAuthenticator.shared.displayStrings.usernamePlaceholder,
    ...
    func configurePasswordTextField(_ cell: TextFieldTableViewCell) {
        cell.configure(withStyle: .password,
                       placeholder: FiableAuthenticator.shared.displayStrings.passwordPlaceholder,
    ...
    func configureForgotPassword(_ cell: TextLinkButtonTableViewCell) {
        cell.configureButton(text: FiableAuthenticator.shared.displayStrings.resetPasswordButtonTitle, accessibilityTrait: .link)
        cell.actionHandler = { [weak self] in
            ...
            FiableAuthenticator.openForgotPasswordURL(self.loginFields)
        }
    }
    ...
    func presentVerifyEmail(loginFields: LoginFields) {
        guard let vc = VerifyEmailViewController.instantiate(from: .verifyEmail) else {
            print("Failed to navigate from SiteCredentialsViewController to VerifyEmailViewController")
            return
        }
    ...
    func makeLoginFieldsUsing(xmlrpc: String, options: [AnyHashable: Any]) -> LoginFields? {
        guard let xmlrpcURL = URL(string: xmlrpc) else {
            print("Failed to initiate XML-RPC URL from \(xmlrpc)")
            return nil
        }
        ...
        guard let email = options["jetpack_user_email"] as? [String: Any],
            ...
            print("Failed to find jetpack_user_email value.")
            return nil
        }
        ...
        guard let home_url = options["home_url"] as? [String: Any],
            ...
            print("Failed to find home_url value.")
            return nil
        }
...
//        loginFields.meta.xmlrpcURL = xmlrpcURL as NSURL
    ...
    func handleLoginFailure(error: Error, incorrectCredentials: Bool) {
        ...
        guard configuration.enableManualErrorHandlingForSiteCredentialLogin == false else {
            FiableAuthenticator.shared.delegate?.handleSiteCredentialLoginFailure(error: error, for: loginFields.siteAddress, in: self)
            return
        }
    ...
    func syncDataOrPresentWPComLogin(with wporgCredentials: WordPressOrgCredentials) {
        ...
        FiableAuthenticator.shared.delegate?.sync(credentials: credentials) { [weak self] in
            NotificationCenter.default.post(name: Foundation.Notification.Name(rawValue: FiableAuthenticator.WPSigninDidFinishNotification), object: nil)
    ...
    func presentWPComLogin(wporgCredentials: WordPressOrgCredentials) {
        ...
        guard let loginFields = makeLoginFieldsUsing(xmlrpc: wporgCredentials.xmlrpc,
            ...
            print("Unexpected response from .org site credentials sign in using XMLRPC.")
            let credentials = AuthenticatorCredentials(wporg: wporgCredentials)
...
extension SiteCredentialsViewController {
    ...
    @objc func sanitizedSiteAddress(siteAddress: String) -> String {
        let baseSiteUrl = FiableAuthenticator.baseSiteURL(string: siteAddress) as NSString
    ...
    @objc override func displayRemoteError(_ error: Error) {
        configureViewLoading(false)
