...
final class VerifyEmailViewController: LoginViewController {
    ...
    override var preferredStatusBarStyle: UIStatusBarStyle {
        FiableAuthenticator.shared.unifiedStyle?.statusBarStyle ?? FiableAuthenticator.shared.style.statusBarStyle
    }
...
private extension VerifyEmailViewController {
    ...
    func configureInstructionLabel(_ cell: TextLabelTableViewCell) {
        cell.configureLabel(text: FiableAuthenticator.shared.displayStrings.verifyMailLoginInstructions,
                            style: .body)
    }
    ...
    func configureTypePasswordButton(_ cell: TextLabelTableViewCell) {
        cell.configureLabel(text: FiableAuthenticator.shared.displayStrings.alternativelyEnterPasswordInstructions,
                            style: .body)
    }
...
//        })
    ...
    func presentUnifiedPassword() {
        guard let vc = PasswordViewController.instantiate(from: .password) else {
            print("Failed to navigate to PasswordViewController from VerifyEmailViewController")
            return
        }
...
private extension VerifyEmailViewController {
    enum ButtonConfiguration {
        enum SendEmailVerificationLink {
            static let title = FiableAuthenticator.shared.displayStrings.sendEmailVerificationLinkButtonTitle
        }

        enum LoginWithAccountPassword {
            static let title = FiableAuthenticator.shared.displayStrings.loginWithAccountPasswordButtonTitle
        }
    }
}
