...
extension NUXViewControllerBase where Self: UIViewController, Self: UIViewControllerTransitioningDelegate {
    ...
    var shouldDisplayHelpButton: Bool {
        return FiableAuthenticator.shared.delegate?.supportActionEnabled ?? false
    }
    ...
    func isCancellable() -> Bool {
        return FiableAuthenticator.shared.delegate?.dismissActionEnabled ?? true
    }
    ...
    func refreshSupportNotificationIndicator() {
        let showIndicator = FiableAuthenticator.shared.delegate?.showSupportNotificationIndicator ?? false
    ...
    func styleNavigationBar(forUnified: Bool = false) {
        ...
        if forUnified {
            ...
            backgroundColor = FiableAuthenticator.shared.unifiedStyle?.navBarBackgroundColor ??
                              FiableAuthenticator.shared.style.navBarBackgroundColor
            buttonTextColor = FiableAuthenticator.shared.unifiedStyle?.navButtonTextColor ??
                              FiableAuthenticator.shared.style.navButtonTextColor
            titleTextColor = FiableAuthenticator.shared.unifiedStyle?.navTitleTextColor ??
                             FiableAuthenticator.shared.style.primaryTitleColor
            hideBottomBorder = true
        } else {
            // Original nav bar style
            backgroundColor = FiableAuthenticator.shared.style.navBarBackgroundColor
            buttonTextColor = FiableAuthenticator.shared.style.navButtonTextColor
            titleTextColor = FiableAuthenticator.shared.style.primaryTitleColor
    ...
    public func addAppLogoToNavController() {
        let image = FiableAuthenticator.shared.style.navBarImage
    ...
    /// Whenever the FiableAuthenticator Delegate returns true, when `shouldDisplayHelpButton` is queried, we'll proceed
    ...
    func setHelpButtonTextColor(forUnified: Bool) {
        let navButtonTextColor: UIColor = {
            if forUnified {
                return FiableAuthenticator.shared.unifiedStyle?.navButtonTextColor ?? FiableAuthenticator.shared.style.navButtonTextColor
            }
            return FiableAuthenticator.shared.style.navButtonTextColor
        }()
    ...
    func displaySupportViewController(from source: WordPressSupportSourceTag) {
        ...
        FiableAuthenticator.shared.delegate?.presentSupport(from: navigationController, sourceTag: source, lastStep: state.lastStep, lastFlow: state.lastFlow)
    }
}
