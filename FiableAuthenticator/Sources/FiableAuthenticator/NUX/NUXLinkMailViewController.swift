...
class NUXLinkMailViewController: LoginViewController {
    ...
    override func viewDidLoad() {
        ...
        imageView.image = FiableAuthenticator.shared.displayImages.magicLink
    ...
    @objc func localizeControls() {
        ...
        label?.text = NSLocalizedString("Check your email on this device, and tap the link in the email you received from hello@fiable.agency.\n\nNot seeing the email? Check your Spam or Junk Mail folder.", comment: "Instructional text on how to open the email containing a magic link.")

        label?.textColor = FiableAuthenticator.shared.style.instructionColor
    }
...
//                case .login:
//                    FiableAuthenticator.track(.loginMagicLinkOpenEmailClientViewed)
//                case .signup:
//                    FiableAuthenticator.track(.signupMagicLinkOpenEmailClientViewed)
//                }
            }
        }
...
//        FiableAuthenticator.track(.loginMagicLinkExited)
        guard let vc = LoginWPComViewController.instantiate(from: .login) else {
            print("Failed to navigate to LoginWPComViewController from NUXLinkMailViewController")
            return
        }
