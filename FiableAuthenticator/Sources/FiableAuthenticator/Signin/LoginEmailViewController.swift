...
open class LoginEmailViewController: LoginViewController, NUXKeyboardResponder {
    ...
    private let showLoginOptions = FiableAuthenticator.shared.configuration.showLoginOptions
...
//        FiableAuthenticator.track(.loginEmailFormViewed)
    ...
    override func loginToSelfHostedSite() {
        guard let vc = LoginSiteAddressViewController.instantiate(from: .login) else {
            print("Failed to navigate from LoginEmailViewController to LoginSiteAddressViewController")
            return
        }
    ...
    func localizeControls() {
        if loginFields.meta.jetpackLogin {
            instructionLabel?.text = FiableAuthenticator.shared.displayStrings.jetpackLoginInstructions
        } else {
            instructionLabel?.text = FiableAuthenticator.shared.displayStrings.emailLoginInstructions
        }
        ...
        button.on(.touchUpInside) { [weak self] (_) in
            ...
            vc.emailTapped = { [weak self] in
                guard let toVC = SignupEmailViewController.instantiate(from: .signup) else {
                    print("Failed to navigate from LoginEmailViewController to SignupEmailViewController")
                    return
                }
    ...
    private func configureAlternativeLabel() {
        ...
        alternativeLoginLabel?.textColor = FiableAuthenticator.shared.style.subheadlineColor
    }
...
//        FiableAuthenticator.track(.loginAutoFillCredentialsFilled)
    }
    ...
    func loginWithUsernamePassword(immediately: Bool = false) {
        if immediately {
            ...
            guard let vc = LoginWPComViewController.instantiate(from: .login) else {
                print("Failed to navigate from LoginEmailViewController to LoginWPComViewController")
                return
            }
    ...
    func requestLink() {
        guard let vc = LoginLinkRequestViewController.instantiate(from: .login) else {
            print("Failed to navigate from LoginEmailViewController to LoginLinkRequestViewController")
            return
        }
    ...
    @objc func showSelfHostedUsernamePasswordAndError(_ error: Error) {
        ...
        guard let vc = LoginSelfHostedViewController.instantiate(from: .login) else {
            print("Failed to navigate from LoginEmailViewController to LoginSelfHostedViewController")
            return
        }
    ...
    private func presentGoogleSignupView() {
        guard let toVC = SignupGoogleViewController.instantiate(from: .signup) else {
            print("Failed to navigate to SignupGoogleViewController from LoginEmailVC")
            return
        }
