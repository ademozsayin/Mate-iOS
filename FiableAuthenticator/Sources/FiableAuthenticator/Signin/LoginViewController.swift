...
//        let configuration = FiableAuthenticator.shared.configuration
...
//    }()
    ...
    var authenticationDelegate: FiableAuthenticatorDelegate {
    ...
    open override var preferredStatusBarStyle: UIStatusBarStyle {
        ...
        return FiableAuthenticator.shared.style.statusBarStyle
    }
...
//        displayError(message: "")
    ...
    func styleBackground() {
        view.backgroundColor = FiableAuthenticator.shared.style.viewControllerBackgroundColor
    }
    ...
    func styleInstructions() {
        ...
        instructionLabel?.textColor = FiableAuthenticator.shared.style.instructionColor
    }
    ...
    func showLoginEpilogue(for credentials: AuthenticatorCredentials) {
        ...
        authenticationDelegate.presentLoginEpilogue(in: navigationController,
                                                    for: credentials,
                                                    source: FiableAuthenticator.shared.signInSource) { [weak self] in
    ...
    @objc dynamic open func displayRemoteError(_ error: Error) {
...
//            FiableAuthenticator.track(.twoFactorCodeRequested)
        }

        let unifiedAuthEnabled = FiableAuthenticator.shared.configuration.enableUnifiedAuth
...
extension LoginViewController {
    ...
    func trackSignIn(credentials: AuthenticatorCredentials) {
        ...
        FiableAuthenticator.track(.signedIn, properties: properties)
...
//                            FiableAuthenticator.track(.signedIn, properties: ["source": source])
//
//                            if AuthenticatorAnalyticsTracker.shared.shouldUseLegacyTracker() {
//                                FiableAuthenticator.track(.loginSocialConnectSuccess)
//                                FiableAuthenticator.track(.loginSocialSuccess)
//                            }
//        }, failure: { error in
//            print("Social Link Error: \(error)")
//            FiableAuthenticator.track(.loginSocialConnectFailure, error: error)
...
//            FiableAuthenticator.track(.loginSocialButtonFailure, properties: ["source": SocialServiceName.apple.rawValue])
//            configureViewLoading(false)
//            return
//        }
//
...
//    }
    ...
    private func presentLogin2FA() {
        ...
        if tracker.shouldUseLegacyTracker() {
//            FiableAuthenticator.track(.loginSocial2faNeeded, properties: properties)
        }
    ...
    private func presentUnified2FA() {
        ...
        guard let vc = TwoFAViewController.instantiate(from: .twoFA) else {
            print("Failed to navigate from LoginViewController to TwoFAViewController")
            return
        }
