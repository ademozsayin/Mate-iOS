...
class LoginPrologueSignupMethodViewController: NUXViewController {
    ...
    private func configureButtonVC() {
        ...
        buttonViewController.setupTopButton(title: loginTitle, isPrimary: false, accessibilityIdentifier: "Sign up with Email Button") { [weak self] in
            ...
            defer {
//                FiableAuthenticator.track(.signupEmailButtonTapped)
            }
...
//                    FiableAuthenticator.track(.signupTermsButtonTapped)
                })
            }

            let safariViewController = SFSafariViewController(url: FiableAuthenticator.shared.configuration.wpcomTermsOfServiceURL)
            safariViewController.modalPresentationStyle = .pageSheet
            self?.present(safariViewController, animated: true, completion: nil)
        }
        buttonViewController.stackView?.insertArrangedSubview(termsButton, at: 0)

        if FiableAuthenticator.shared.configuration.enableSignInWithApple {
            buttonViewController.setupTertiaryButtonFor(socialService: .apple) { [weak self] in
                self?.handleAppleButtonTapped()
            }
        }

        buttonViewController.backgroundColor = FiableAuthenticator.shared.style.buttonViewBackgroundColor
    }
...
//            FiableAuthenticator.track(.signupSocialButtonTapped, properties: ["source": "apple"])
        })
...
//            FiableAuthenticator.track(.signupSocialButtonTapped, properties: ["source": "google"])
        })
...
//        FiableAuthenticator.track(.signupCancelled)
        dismiss(animated: true)
    }
