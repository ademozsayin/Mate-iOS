// MARK: - FiableAuthenticator Delegate Protocol
//
...
public protocol FiableAuthenticatorDelegate: AnyObject {
    ...
    func checkIfEmailExist(email: String,  onCompletion: @escaping (Result<EmailCheckData, Error>) -> Void)
    
    func loginWith(email: String, password:String,  onCompletion: @escaping (Result<OnsaTokenData, Error>) -> Void) async
...
///
public extension FiableAuthenticatorDelegate {
    func troubleshootSite(_ siteInfo: WordPressComSiteInfo?, in navigationController: UINavigationController?) {
        // No-op
    }
